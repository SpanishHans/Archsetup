...
live_command_output() {
    ...
    enable_nobody_pacman() {
        if ! sudo grep -q "^nobody ALL=(ALL) NOPASSWD: /usr/bin/pacman" /etc/sudoers; then
            echo "nobody ALL=(ALL) NOPASSWD: /usr/bin/pacman" | sudo EDITOR='tee -a' visudo
        fi
        # scroll_window_output return_value "$(terminal_title "Activated nobody on pacman")" "/etc/sudoers"
    }
